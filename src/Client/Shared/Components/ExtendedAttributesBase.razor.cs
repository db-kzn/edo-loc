...
namespace EDO_FOMS.Client.Shared.Components
{
        ...
        where TId : IEquatable<TId>
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            _canViewExtendedAttributes = (await _authService.AuthorizeAsync(_currentUser, ExtendedAttributesViewPolicyName)).Succeeded;
            if (!_canViewExtendedAttributes)
            {
                _snackBar.Add(_localizer["Not Allowed."], Severity.Error);
                _navigationManager.NavigateTo("/");
            }
            _canEditExtendedAttributes = (await _authService.AuthorizeAsync(_currentUser, ExtendedAttributesEditPolicyName)).Succeeded;
            _canCreateExtendedAttributes = (await _authService.AuthorizeAsync(_currentUser, ExtendedAttributesCreatePolicyName)).Succeeded;
            _canDeleteExtendedAttributes = (await _authService.AuthorizeAsync(_currentUser, ExtendedAttributesDeletePolicyName)).Succeeded;
            _canExportExtendedAttributes = (await _authService.AuthorizeAsync(_currentUser, ExtendedAttributesExportPolicyName)).Succeeded;
            _canSearchExtendedAttributes = (await _authService.AuthorizeAsync(_currentUser, ExtendedAttributesSearchPolicyName)).Succeeded;
