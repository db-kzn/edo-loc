; (function () {
    //already loaded
    if (window.cpcsp_chrome_nmcades) { return; }
    ...
    function ReleasePluginObjects() {
        ...
        return requestPromise.then(function (result) { return (result.data.value == 0); });
    }
    ...
    function CallMethod() {
        ...
        object_messsage = {
            destination: "nmcades", requestid: g_request_id, objid: this["objid"], method: arguments[0],
            params: args
        };
        cpcsp_console_log(cadesplugin.LOG_LEVEL_DEBUG, "nmcades_plugin_api.js: Sent message:" + JSON.stringify(object_messsage));
        const requestPromise = new Promise(function (resolve, reject) {
    ...
    function Json_to_javascript(data) {
        ...
        if (data.retval.type === "string") { return data.retval.value; }
        if (data.retval.type === "number") { return parseInt(data.retval.value); }
        if (data.retval.type === "boolean") { return Boolean(data.retval.value); }
        if (data.retval.type === "OK") { return; }
    }
    ...
    function EnableInternalCSPListener(event) {
        if (event.source !== window) return;
        if (event.data === "EnableInternalCSP_request") window.postMessage("EnableInternalCSP=" + cadesplugin.EnableInternalCSP, "*");
    }
    function storeCallback(event) {
        if (event.source === window && event.data.object !== undefined) {
    ...
    function Print2Digit(digit) { return (digit < 10) ? "0" + digit : digit; }
