...
using EDO_FOMS.Client.Infrastructure.Managers.Communication;
using EDO_FOMS.Shared.Constants.Storage;

namespace EDO_FOMS.Client.Pages.Communication
{
    public partial class Chat
    {
        ...
        private async Task SubmitAsync()
        {
            if (!string.IsNullOrEmpty(CurrentMessage) && !string.IsNullOrEmpty(CId))
            {
                ...
                if (response.Succeeded)
                {
                    ...
                    var userName = $"{user.GetSurname} {user.GetGivenName()}";
                    await HubConnection.SendAsync(AppConstants.SignalR.SendMessage, chatHistory, userName);
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            HubConnection.On<string>(AppConstants.SignalR.ConnectUser, (userId) =>
            {
            ...
            HubConnection.On<string>(AppConstants.SignalR.DisconnectUser, (userId) =>
            {
            ...
            HubConnection.On<ChatHistory<IChatUser>, string>(AppConstants.SignalR.ReceiveMessage, async (chatHistory, userName) =>
             {
                 if ((CId == chatHistory.ToUserId && CurrentUserId == chatHistory.FromUserId) || (CId == chatHistory.FromUserId && CurrentUserId == chatHistory.ToUserId))
                 {
                     if ((CId == chatHistory.ToUserId && CurrentUserId == chatHistory.FromUserId))
                     {
                         ...
                         await HubConnection.SendAsync(AppConstants.SignalR.SendChatNotification, string.Format(_localizer["New Message From {0}"], userName), CId, CurrentUserId);
                     }
        ...
        private async Task LoadUserChat(string userId)
        {
            ...
            if (response.Succeeded)
            {
                ...
                CId = contact.Id;
                CFullName = $"{contact.GivenName} {contact.Surname}";
