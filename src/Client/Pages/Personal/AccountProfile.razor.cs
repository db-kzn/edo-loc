...
namespace EDO_FOMS.Client.Pages.Personal
{
    public partial class AccountProfile
    {
        ...
        private async Task LoadDataAsync()
        {
            ...
            //await _jsRuntime.InvokeVoidAsync("azino.Console", user);

            UserId = user.GetUserId();

            _profileModel.UserId = UserId;
            _profileModel.UserName = user.GetUserName();
            _profileModel.Title = user.GetTitle();

            _profileModel.Surname = user.GetSurname();
            _profileModel.GivenName = user.GetGivenName();

            _profileModel.Snils = user.GetSnils();
            _profileModel.Inn = user.GetInn();

            _profileModel.PhoneNumber = user.GetPhoneNumber();
            _profileModel.Email = user.GetEmail();

            _profileModel.BaseRole = (UserBaseRoles)Enum.Parse(typeof(UserBaseRoles), user.GetBaseRole());
            _profileModel.OrgType = (OrgTypes)Enum.Parse(typeof(OrgTypes), user.GetOrgType());

            var data = await _accountManager.GetProfilePictureAsync(UserId);
            if (data.Succeeded)
            {
                ImageDataUrl = String.IsNullOrEmpty(data.Data) ? string.Empty : data.Data.Replace('\\', '/');
            }
            //if (_profileModel.GivenName.Length > 0)
            //{
            //    _firstLetterOfName = _profileModel.GivenName[0];
            //}
        }
        ...
        private async Task UploadFiles(InputFileChangeEventArgs e)
        {
            ...
            if (_file != null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    _navigationManager.NavigateTo("/personal/account", true);
                }
                else
                {
