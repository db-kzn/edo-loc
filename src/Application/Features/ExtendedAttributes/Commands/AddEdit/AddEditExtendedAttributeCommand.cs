...
namespace EDO_FOMS.Application.Features.ExtendedAttributes.Commands.AddEdit
{
            ...
            where TId : IEquatable<TId>
    {
        ...
        public async Task<Result<TId>> Handle(AddEditExtendedAttributeCommand<TId, TEntityId, TEntity, TExtendedAttribute> command, CancellationToken cancellationToken)
        {
            ...
            if (command.Id.Equals(default))
            {
                ...
                await _unitOfWork.CommitAndRemoveCache(cancellationToken, AppConstants.Cache.GetAllEntityExtendedAttributesCacheKey(typeof(TEntity).Name));
                ...
                var cacheKeys = await _unitOfWork.Repository<TExtendedAttribute>().Entities.Select(x =>
                    AppConstants.Cache.GetAllEntityExtendedAttributesByEntityIdCacheKey(
