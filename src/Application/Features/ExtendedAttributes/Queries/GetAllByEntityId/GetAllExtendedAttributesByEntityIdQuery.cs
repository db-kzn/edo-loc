...
namespace EDO_FOMS.Application.Features.ExtendedAttributes.Queries.GetAllByEntityId
{
            ...
            where TId : IEquatable<TId>
    {
        ...
        public async Task<Result<List<GetAllExtendedAttributesByEntityIdResponse<TId, TEntityId>>>> Handle(GetAllExtendedAttributesByEntityIdQuery<TId, TEntityId, TEntity, TExtendedAttribute> request, CancellationToken cancellationToken)
        {
            ...
            var extendedAttributeList = await _cache.GetOrAddAsync(AppConstants.Cache.GetAllEntityExtendedAttributesByEntityIdCacheKey(typeof(TEntity).Name, request.EntityId), getAllExtendedAttributesByEntityId);
