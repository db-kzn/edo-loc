...
namespace EDO_FOMS.Application.Features.ExtendedAttributes.Queries.GetAll
{
            ...
            where TId : IEquatable<TId>
    {
        ...
        public async Task<Result<List<GetAllExtendedAttributesResponse<TId, TEntityId>>>> Handle(GetAllExtendedAttributesQuery<TId, TEntityId, TEntity, TExtendedAttribute> request, CancellationToken cancellationToken)
        {
            ...
            var extendedAttributeList = await _cache.GetOrAddAsync(AppConstants.Cache.GetAllEntityExtendedAttributesCacheKey(typeof(TEntity).Name), getAllExtendedAttributes);
