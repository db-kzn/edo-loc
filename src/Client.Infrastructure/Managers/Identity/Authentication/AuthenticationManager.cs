...
using EDO_FOMS.Client.Infrastructure.Routes;
...
namespace EDO_FOMS.Client.Infrastructure.Managers.Identity.Authentication
{
    public class AuthenticationManager : IAuthenticationManager
    {
        ...
        public async Task<IResult> Login(TokenRequest model)
        {
            ...
            ((AuthStateProvider)this._authStateProvider).MarkUserAsAuthenticated(model.Email);
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

            return await Result.SuccessAsync();
        }

        public async Task<IResult> SignIn(CertCheckRequest model)
        {
            var response = await _httpClient.PostAsJsonAsync(TokenEndpoints.PostByCert, model);
            var result = await response.ToResult<TokenResponse>();
            ...
            await _localStorage.SetItemAsync(StorageConstants.Local.AuthToken, res.Token);
            await _localStorage.SetItemAsync(StorageConstants.Local.RefreshToken, res.RefreshToken);

            if (!string.IsNullOrEmpty(res.UserImageURL))
            {
                await _localStorage.SetItemAsync(StorageConstants.Local.UserImageURL, res.UserImageURL);
            }

            ((AuthStateProvider)this._authStateProvider).MarkUserAsAuthenticated(res.UserName);
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", res.Token);

            return await Result.SuccessAsync();
        }

        public async Task<IResult<CertCheckResponse>> CertCheck(CertCheckRequest model)
        {
            var response = await _httpClient.PostAsJsonAsync(TokenEndpoints.PostCheckCert, model);
            return await response.ToResult<CertCheckResponse>(); ;
        }
